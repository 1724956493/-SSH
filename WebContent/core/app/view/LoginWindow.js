Ext.define('core.app.view.LoginWindow', {
	...
	listeners : {
		show:function(_this){
			...
			_this.down("form").getForm().findField("password").setValue(passWord);
			...
			items : [ {
				...
				listeners:{
					change: function(_this,newV,oldV){
						var password = _this.up("form").getForm().findField("password");
						if(Ext.util.Cookies.get("username") != null){
							if(newV == Ext.util.Cookies.get("username")){
								password.setValue(Ext.util.Cookies.get("password"));
							}else{
								password.setValue("");
							}
						}
